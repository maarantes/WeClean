...
export const registrarTarefaNoCalendario = async (
  tarefa: Tarefa
): Promise<void> => {
  const now = new Date();
  ...
  for (const date of occurrenceDates) {
    ...
    const instanciaCol = collection(calendarRef, "instancias");
    const instanceRef  = doc(instanciaCol);
