...
import ConvidarModal from "../../components/Modals/ModalConvidar"
import EntrarGrupoModal from "../../components/Modals/ModalEntrarGrupo"
...
import RenomearGrupoModal from "@/frontend/components/Modals/ModalRenomearGrupo"
import AlertaSimples from "@/frontend/components/AlertaSimples"
import KickIntegranteModal from "@/frontend/components/Modals/ModalTirarIntegrante"
import { kickarIntegrante } from "@/backend/services/grupos/removerIntegrante"
import ModalSairGrupo from "@/frontend/components/Modals/ModalSairGrupo"
import ModalExcluirGrupo from "@/frontend/components/Modals/ModalExcluirGrupo"
  ...
  return (
    <>
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: "white" }}>
          ...
          >
            ...
            <View style={styles.card_grupo}>
              ...
              <Text style={styles.texto_integrantes}>
                {integrantesCount.toString().padStart(2, "0")}{" "}
                {integrantesCount === 1 ? "integrante" : "integrantes"}
              </Text>
            </View>
            ...
            <View style={styles.botoes_container}>
              ...
              >
                ...
                <Text style={[styles.botao_base_texto, styles.botao_convidar_texto]}>Convidar</Text>
              </TouchableOpacity>
            ...
            <View style={styles.container_integrantes}>
              ...
              {integrantes
                ...
                .map((pessoa, index) => {
                  const bgClass = globalStyles[
                    `tema_bg_${pessoa.tema}_secundario` as keyof typeof globalStyles
                  ] as { backgroundColor: string }
                  const colorClass = globalStyles[
                    `tema_color_${pessoa.tema}_primario` as keyof typeof globalStyles
                  ] as { color: string }

                  if (pessoa.tipo === "admin") {
                    return (
                      <View key={index} style={[styles.container_pessoa_admin, bgClass]}>
          ...
          onConfirmKick={async () => {
            try {
              if (integranteSelecionadoUID) {
                ...
                setMensagemToast(
                  `"${integranteSelecionadoNome}" foi removido do grupo com sucesso!`
                )
                setToastVisible(true)
                carregarGrupo()
              }
        ...
        <ModalSairGrupo visible={sairGrupoModalVisible} setVisible={setSairGrupoModalVisible} />
